...
"DT_node_lookup_1" AS 
 (SELECT
    t."index" AS "index",
    CASE WHEN (t."X_2" < 5.000000) THEN CASE WHEN (t."X_2" < 1.700000) THEN 3 ELSE 4 END ELSE 2 END AS node_id
  FROM model_input t
 )
,
"DT_node_data_1" AS 
 (SELECT
  ...
  FROM (
    ...
    UNION ALL
    SELECT 3 AS nid,  0.304653 AS "P_0", -0.179481 AS "P_1", -0.186104 AS "P_2"
    UNION ALL
    SELECT 4 AS nid,  -0.105657 AS "P_0", 0.241232 AS "P_1", -0.158705 AS "P_2"
  ) AS "Values")
,
"DT_output_1" AS 
 (SELECT
...
"DT_node_data_2" AS 
 (SELECT
  ...
  FROM (
    ...
    UNION ALL
    SELECT 3 AS nid,  0.239293 AS "P_0", -0.162829 AS "P_1", -0.167260 AS "P_2"
    UNION ALL
    SELECT 4 AS nid,  -0.087126 AS "P_0", 0.181367 AS "P_1", -0.139888 AS "P_2"
  ) AS "Values")
,
"DT_output_2" AS 
 (SELECT
...
"DT_node_lookup_3" AS 
 (SELECT
    t."index" AS "index",
    CASE WHEN (t."X_3" < 1.800000) THEN CASE WHEN (t."X_0" < 5.700000) THEN 3 ELSE 4 END ELSE 2 END AS node_id
  FROM model_input t
 )
,
"DT_node_data_3" AS 
 (SELECT
    ...
    "Values"."P_0" AS "P_0", "Values"."P_1" AS "P_1", "Values"."P_2" AS "P_2"
  FROM (
    SELECT 2 AS nid,  -0.153028 AS "P_0", -0.163972 AS "P_1", 0.209091 AS "P_2"
    UNION ALL
    SELECT 3 AS nid,  0.174261 AS "P_0", -0.081374 AS "P_1", -0.160924 AS "P_2"
    UNION ALL
    SELECT 4 AS nid,  -0.118419 AS "P_0", 0.160988 AS "P_1", -0.086940 AS "P_2"
  ) AS "Values")
,
"DT_output_3" AS 
 (SELECT
...
"DT_node_data_4" AS 
 (SELECT
    ...
    "Values"."P_0" AS "P_0", "Values"."P_1" AS "P_1", "Values"."P_2" AS "P_2"
  FROM (
    SELECT 1 AS nid,  0.065006 AS "P_0", 0.039038 AS "P_1", -0.137247 AS "P_2"
    UNION ALL
    SELECT 2 AS nid,  -0.144166 AS "P_0", -0.143599 AS "P_1", 0.178258 AS "P_2"
  ) AS "Values")
,
"DT_output_4" AS 
 (SELECT
